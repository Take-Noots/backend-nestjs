...
export class SpotifySessionService {
  ...
  private readonly JWT_SECRET: string = process.env.JWT_SECRET || 'default-jwt-secret';
  private readonly ENCRYPTION_KEY: string = process.env.SPOTIFY_ENCRYPTION_KEY || 'default-encryption-key-32-chars-long!';
  private readonly ENCRYPTION_IV: string = process.env.SPOTIFY_ENCRYPTION_IV || 'default-iv-16chr';
  ...
  storeAccessToken(userId: string, accessToken: string, expiresIn: number): void {
  ...
  async saveRefreshToken(userId: string, refreshToken: string): Promise<SpotifySession> {
    const encryptedToken = this.encryptToken(refreshToken);

    // Try to find existing session for this user
    const existingSession = await this.sessionModel.findOne({ user_id: userId }).exec();
